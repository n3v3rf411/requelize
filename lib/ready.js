  ...
  return ensureDb(requelize)
    .then(() => {
      const modelsReady = Object
        ...
        .map((name) => {
          // Update belongsToMany with custom tables to add JoinModel
          const joins = requelize.models[name]._joins
          Object.keys(joins)
            .filter(field => joins[field].type === 'belongsToMany' && !joins[field].JoinModel)
            .forEach((field) => {
              joins[field].JoinModel = requelize.models[joins[field].tableName]
            })

          return requelize.models[name].ready()
        })
